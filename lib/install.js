...
function syncWidgetSets(onlyLocal, enabledList) {
    ...
    if (onlyLocal) {
        _path = path.normalize(`${__dirname}/../node_modules/`);
    } else {
        _path = path.normalize(`${__dirname}/../../`);
    }
    let dirs = fs.readdirSync(_path);
    const sets = [];
    for (let d = 0; d < dirs.length; d++) {
        if (dirs[d].match(/^iobroker\./i) && fs.existsSync(`${_path + dirs[d]}/widgets/`)) {
            pack = null;
            try {
                pack = JSON.parse(fs.readFileSync(`${_path + dirs[d]}/io-package.json`).toString());
            } catch (e) {
                console.warn(`Cannot parse "${_path}${dirs[d]}/io-package.json": ${e}`);
            }
            sets.push({path: _path + dirs[d], name: dirs[d].toLowerCase(), pack: pack});
        }
    }
    if (!onlyLocal) {
        try {
            dirs = fs.readdirSync(path.normalize(`${__dirname}/../../../../`));
            for (let d = 0; d < dirs.length; d++) {
                if (dirs[d].match(/^iobroker\./i) && fs.existsSync(path.normalize(`${__dirname}/../../../../${dirs[d]}/widgets/`))) {
                    found = false;
                    ...
                    if (!found) {
                        pack = null;
                        try {
                            pack = JSON.parse(fs.readFileSync(path.normalize(`${__dirname}/../../../../${dirs[d]}/io-package.json`)).toString());
                        } catch (e) {
                            console.warn(`Cannot parse "${path.normalize(`${__dirname}/../../../../${dirs[d]}`)}/io-package.json": ${e}`);
                        }
                        sets.push({path: path.normalize(`${__dirname}/../../../../${dirs[d]}`), name: dirs[d].toLowerCase()});
                    }
                }
            }
        } catch (e) {
    ...
    for (let d = 0; d < sets.length; d++) {
        ...
        let _changed = copyFolderRecursiveSync(`${sets[d].path}/widgets/`, path.normalize(`${__dirname}/../www/`));
        if (_changed) {
            filesChanged = true;
        }
