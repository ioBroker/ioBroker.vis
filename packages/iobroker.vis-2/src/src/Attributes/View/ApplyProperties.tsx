...
export function renderApplyDialog(props: RenderApplyDialogProps) {
    if (!props.field) {
        return null;
    }
    const {
    ...
    for (let f = 0; f < field.group.fields.length; f++) {
        ...
        if (cField.applyToAll) {
            const viewsToChangeForOne: string[] | null = getViewsWithDifferentValues(project, cField, selectedView, viewList, checkFunction) || [];
        ...
        onClose={result => {
            if (result) {
                ...
                for (let f = 0; f < field.group.fields.length; f++) {
                    ...
                    if (cField.applyToAll) {
                        const _viewsToChange: string[] | null = getViewsWithDifferentValues(project, field, selectedView, viewList, checkFunction) || [];
                        _viewsToChange?.forEach(_view => {
                            newProject[_view].settings[cField.attr] = newProject[selectedView].settings[cField.attr];
                        });
                    }
                }
