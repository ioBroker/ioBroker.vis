...
vis = $.extend(true, vis, {
    styleSelect: {
        ...
        Show: function (options) {
            ...
            htmlElem.settings.styles[vis.translate("Default")] = {style: null, file: null};
	...
	imageSelect: {
		...
		Show:  function (options) {
			...
			if (this._selectText == "") {
				this._selectText = vis.translate("Select");
				this._mkdirText = vis.translate("new Folder");
				this._cancelText = vis.translate("Cancel");
				this._uploadText = vis.translate("Upload");
				this._titleText  = vis.translate("Selected image: ");
			}
			...
			dialog_buttons[this._selectText] = function () {
				...
				if (this.settings.onselect) {
                    this.settings.onselect(vis.imageSelect._pictDir + this.settings.result, this.settings.onselectArg);
                }
			};
            ...
            $(htmlElem).dialog({
				...
				close: function () {
                    ...
                    if (!vis.imgSelectCache) {
                        vis.imgSelectCache = [];
                    }
                    var preloadArr = [];
                    $(this).find("img").each(function () {
                        var src = $(this).attr("src");
                        if (vis.imgSelectCache.indexOf(src) == -1) {
                            vis.imgSelectCache.push(src);
                            preloadArr.push(src);
                        }
                    });
                    vis.preloadImages(preloadArr);
                    */
                    $(this).remove();
                },
			...
			$(htmlElem).dropzone({
				...
				complete: function (e) {
					if (this.element.settings.onselect) {
						this.element.settings.onselect("img/"+vis.imageSelect._curDir+ e.name, this.element.settings.onselectArg);
					}
				...
				init: function () {
					this.on("processing", function () {
						this.options.url = "/upload?path=./www/dashui/img/"+vis.imageSelect._curDir;
					});
				}
			});
		...
		getFileList: function (htmlElem) {
			// find selected image
			vis.imageSelect._curImage = "";
			
			if (htmlElem.settings.result && htmlElem.settings.result != "") { 
				var str = htmlElem.settings.result;
				if (str.substring (0, vis.imageSelect._pictDir.length) == vis.imageSelect._pictDir) {
					str = str.substring(vis.imageSelect._pictDir.length);
				}
				if  (str.substring (0, vis.imageSelect._curDir.length) == vis.imageSelect._curDir) {
					str = str.substring(vis.imageSelect._curDir.length);
					if (str.indexOf('/') == -1) {
						vis.imageSelect._curImage = str;
					}
				}
			}
			
			// Load directory
			vis.conn.readDir(this._rootDir + this._curDir, function (dirArr) {
				vis.imageSelect.showImages(dirArr, htmlElem);
			});
		},
		showImages: function (aImages, obj) {	
			...
			var bImages = [];
			var j = 0;
			if (vis.imageSelect._curDir != null && vis.imageSelect._curDir != "") {
				bImages[j++] = "..";
			}
			...
			for (var i = 0; i < aImages.length; i++) {
				if (aImages[i].indexOf('.') != -1 && aImages[i] != vis.imageSelect._dirImage) {
					bImages[j++] = aImages[i];
				}
			}
			...
			for (row = 0; row < obj.settings.rows; row++) {
				...
				for (col = 0; col < obj.settings.columns; col++) {
					if (aImages.length > id) {
						...
						// File or directory
						if (aImages[id] == "..") {
							sText += " src=\""+vis.imageSelect._pictDir + vis.imageSelect._dirImage + "\" title='" + vis.translate("Back") + "'";
						} else if (isDir) {
							sText += " src=\"" + vis.imageSelect._pictDir+vis.imageSelect._dirImage + "\" title='" + aImages[id] + "' ";
						} else if (aImages[id].indexOf(".wav") != -1 || aImages[id].indexOf(".mp3") != -1) {
							sText += " src=\"" + vis.imageSelect._pictDir+vis.imageSelect._soundImage + "\" title='"+aImages[id] + "' ";
						} else {
			...
			for (i = 0; i < aImages.length; i++) {
				...
				img.image = image;
				if (vis.imageSelect._curImage == aImages[i]) {	
				...
				img.bind("click", {msg: img}, function (event) {			
					...
					// back directory
					if (obj_.result == "..") {
						var dirs = vis.imageSelect._curDir.split ('/');
						vis.imageSelect._curDir = "";
						for (var t = 0; t < dirs.length - 2; t++)
							vis.imageSelect._curDir += dirs[t]+"/";
						vis.imageSelect.getFileList (obj);
					} else if (obj_.result.indexOf('.') == -1) {
						vis.imageSelect._curDir += obj_.result+"/";
						vis.imageSelect.getFileList (obj);
					} else {
						obj.settings.result = vis.imageSelect._curDir+obj_.result;
						...
						$(obj).dialog('option', 'title', vis.imageSelect._titleText + obj.settings.result);
					}
				});				
				img.bind("dblclick", {msg: img}, function (event) {
					var obj_ = event.data.msg;
					obj.settings.result = vis.imageSelect._pictDir + vis.imageSelect._curDir + obj_.result;
				...
				if (aImages[i] != ".." && aImages[i].indexOf('.') != -1 && aImages[i].indexOf(".wav") == -1 && aImages[i].indexOf(".mp3") == -1) {
					image.attr('src', vis.imageSelect._pictDir+vis.imageSelect._curDir+aImages[i]);
				}
			}
			// Show active image
			if (vis.imageSelect._curImage != null && vis.imageSelect._curImage != "") { 
				$(obj).dialog('option', 'title', vis.imageSelect._titleText + vis.imageSelect._curDir + vis.imageSelect._curImage);
			} else {
				$(obj).dialog('option', 'title', vis.imageSelect._titleText + vis.imageSelect._curDir);
			}
		},
...
var colorSelect = {
    ...
    Show:  function (options) {
        ...
        if (this._selectText == "") {
            this._selectText = vis.translate("Select");
            this._cancelText = vis.translate("Cancel");
            this._titleText  = vis.translate("Select color");
        }
...
var hmSelect = {
    ...
    _type2Str: function (type, subtype) {
        ...
        switch (type) {
            case 2:
                if (subtype == 6) {
                    return vis.translate('Alarm');
                } else if (subtype == 2) {
                    return vis.translate('Logical');
                } else {
                    return vis.translate('Boolean') + "," + subtype;
                }

            case 20:
                if (subtype == 11) {
                    return vis.translate('String');
                } else {
                    return vis.translate('String') + "," + subtype;
                }
            case 4:
                if (subtype == 0) {
                    return vis.translate('Number');
                } else {
                    return vis.translate('Number') + "," + subtype;
                }

            case 16:
                if (subtype == 29) {
                    return vis.translate('Enum');
                } else {
                    return vis.translate('Enum') + "," + subtype;
                }
            default:
                return ''+type+","+subtype;
        }
    ...
    _buildVarsGrid: function (localData) {
		...
		$("#hmVarsContent").jqGrid({
			...
			autowidth:   true,
			shrinkToFit: false,
			colNames:['Id', vis.translate('Name'), '', vis.translate('Type'), vis.translate('Unit'), vis.translate('Description'), ''],
			colModel:[
    ...
    _buildProgsGrid: function (localData) {
		...
		$("#hmProgsContent").jqGrid({
			...
			autowidth:   true,
			shrinkToFit: false,
			colNames:['Id', vis.translate('Name'), '', vis.translate('Description'), ''],
			colModel:[
                {name:'id',          index:'id',          width:1,   hidden:true, key:true},
    ...
    _buildDevicesGrid: function (localData, filter, devFilter) {
        ...
        if (1) {
            ...
            var text = vis.translate('Rooms')+":&nbsp;<select id='hmSelectLocations'>";
                text += "<option value=''>"+vis.translate('All')+"</option>";
            for (var room in rooms) {
                var selected = "";
            ...
            text += vis.translate('Functions')+":&nbsp;<select id='hmSelectFunctions'>";
            text += "<option value=''>"+vis.translate('All') + "</option>";
            for (var func in functions) {
                var selected = "";
            ...
            if (filter != "all" || devFilter != null || devFilter != "") {
                text += vis.translate("Disable device filter:") + "<input type='checkbox' id='hmSelectIgnoreFilter' "+ (this._ignoreFilter ? "checked" : "") + ">";
            }
        ...
        if (isLocations) {
            ...
            colNames = ['Id', vis.translate('Name'), '', vis.translate('Location'), vis.translate('Interface'), vis.translate('Type'), vis.translate('Function'), vis.translate('Address'), ''];
        } else {
            colModel = [
                {name:'id',       index:'id',        width:1,   hidden:true, key:true},
                {name:'Name',     index:'Name',      width:250, sortable:"text"},
            ...
            colNames = ['Id', vis.translate('Name'), '', vis.translate('Value'), vis.translate('Interface'), vis.translate('Type'), vis.translate('Function'), vis.translate('Address'), ''];
    ...
    show: function (localData, userArg, onSuccess, filter, devFilter) { // onsuccess (userArg, value, valueObj)  
		...
		if (!document.getElementById("hmSelect")) {
			$("body").append("<div class='dialog' id='hmSelect' title='" + vis.translate("Select HM parameter") + "'></div>");
            ...
            if (devFilter === undefined || devFilter == "" || this._ignoreFilter) {
                if (filter == 'all' || this._ignoreFilter || (filter != 'variables' && filter != 'programs' && filter != 'devices')) {
                    text += "    <li><a href='#tabs-devs'  id='dev_select'>"+vis.translate("Devices")+"</a></li>";
                }
                if (this._ignoreFilter || ((devFilter === undefined || devFilter == "") && (filter == 'all' || filter == 'variables'))) {           
                    text += "    <li><a href='#tabs-vars'  id='var_select'>"+vis.translate("Variables")+"</a></li>";
                }
                if (this._ignoreFilter || ((devFilter === undefined || devFilter == "") && ( filter == 'all' || filter == 'programs'))) {           
                    text += "    <li><a href='#tabs-progs' id='prog_select'>"+vis.translate("Programs")+"</a></li>";
                }
                text += "  </ul>";
            }
...
vis.useNewSelectDialog =  (!$().jqGrid);// || true),
...
var idSelect = {
    ...
    _buildTable: function (divName) {
        ...
        $('.no-select-tr').click(function () {
            ...
            $('#idSelect').dialog('option', 'title', vis.translate("Select ID") + ': ' + that._locData.metaObjects[idSelect._options.selectedID].name);
    ...
    Show: function (locData, options) {
        ...
        if (!document.getElementById("idSelect")) {
            ...
            $('body').append(
                    '<div class="dialog" id="idSelect" title="' + vis.translate('Select ID') + selName +'">' +
